...
// ─── 관리자 라우트 초기 방어층 ─────────────────────────────
// /_c/<seg> 라우트가 존재 자체를 은닉하도록 middleware 단에서 세그먼트 유효성 검사.
...
// - env 미설정 상태에서는 어떤 세그먼트도 유효하지 않으므로 /_c/* 전체가 404.
//
// 진짜 권한 검증(staff.admin)은 layout의 isCurrentUserAdmin()이 담당.
// 여기선 "존재 은닉" 층만.
const ADMIN_PREFIX = "/_c/";

const isValidAdminPath = (pathname: string): boolean => {
  if (!pathname.startsWith(ADMIN_PREFIX)) return true; // /_c/* 아니면 pass
